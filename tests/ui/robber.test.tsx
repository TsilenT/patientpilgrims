...
test("clicking a robber hex with one victim waits for confirmation before stealing", async () => {
  ...
  const edge = topology().hexEdges.get(targetHex)![0]!;
  g.board.buildings[vtx] = { owner: 1, type: "settlement" };
  g.board.roads[edge] = { owner: 1 };
  ...
  const selectedSlot = container.querySelector(`[data-hex-slot="${targetHex}"]`)!;
  const selectedDash = container.querySelector(".hex-select-dash")!;
  const road = container.querySelector(`[data-road="${edge}"]`)!;
  const building = container.querySelector(`[data-building="${vtx}"]`)!;
  expect(selectedSlot.getAttribute("data-selected")).toBe("true");
  expect(selectedSlot.compareDocumentPosition(road) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  expect(selectedSlot.compareDocumentPosition(building) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  expect(road.compareDocumentPosition(selectedDash) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  expect(building.compareDocumentPosition(selectedDash) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
